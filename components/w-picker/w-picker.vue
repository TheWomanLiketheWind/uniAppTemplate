<template>
  <view class="w-picker">
    ...
    <view class="w-picker-cnt"
          :class="{'show':showPicker}">
      <view class="w-picker-hd"
            @touchmove.stop.prevent
            catchtouchmove="true">
        <view class="w-picker-btn"
              @tap="pickerCancel">取消</view>
        <view class="w-picker-btn"
              :style="{'color':themeColor}"
              @tap="pickerConfirm">确定</view>
      </view>
      <view class="w-picker-view"
            v-if="mode=='date'||mode=='dateTime'||mode=='yearMonth'">
        <picker-view :indicator-style="itemHeight"
                     :value="pickVal"
                     @change="bindChange">
          <picker-view-column>
            <view class="item"
      ...
      <view class="w-picker-view"
        ...
        <picker-view :indicator-style="itemHeight"
          ...
          <picker-view-column>
            <view class="item"
                  v-for="(item,index) in data.hours"
                  :key="index">{{item.label}}时</view>
          </picker-view-column>
          <picker-view-column>
            <view class="item"
                  v-for="(item,index) in data.minutes"
                  :key="index">{{item.label}}分</view>
          </picker-view-column>
        </picker-view>
      </view>
    </view>
...
import { initRange, initRangeDays } from "@/common/range.js";
...
export default {
  ...
  methods: {
    ...
    bindChange(val) {
      let valData = val.detail.value
      console.log(this.data.provinces)
      console.log(valData)

      this.cityChange(this.data.provinces[valData[0]].id)
      let _this = this;
      ...
      switch (mode) {
        case "limit":
          ...
          if (col1.flag) {
            _this.data = initBefore(_this.dayStep, _this.starHour, _this.endHour, _this.minuteStep, _this.afterStep);
            col2 = _this.data.hours[arr[1]];
            if (!col2.flag) {
              let minutes = initMinutes(_this.minuteStep);
              _this.data.minutes = minutes;
            } else {
              _this.data = initBefore(_this.dayStep, _this.starHour, _this.endHour, _this.minuteStep, _this.afterStep);
            };
          } else {
            let hours = initHours(_this.startHour, _this.endHour);
            let minutes = initMinutes(_this.minuteStep);
        ...
        case "range":
          ...
          if (fyear != checkArr[0]) {
            days = initRangeDays(fyear, fmonth);
          ...
          if (tyear != checkArr[3]) {
            days = initRangeDays(tyear, tmonth);
        ...
        case "time":
          ...
          break;
        case "region":
          province = _this.data.provinces[arr[0]].name;
          city = _this.data.citys[arr[1]].name;
          area = _this.data.areas[arr[2]].name;
          if (province != checkArr[0]) {
            // _this.data.citys = citys[arr[0]];
            // _this.data.areas = areas[arr[0]][0];
            arr[1] = 0;
            arr[2] = 0;
            city = _this.data.citys[arr[1]];
            area = _this.data.areas[arr[2]];
          };
          if (city != checkArr[1]) {
        ...
        case "selector":
          ...
          _this.resultStr = _this.data[arr[0]];
          break;
      }
      _this.$nextTick(() => {
        //_this.pickVal=arr;
    ...
    initData() {
      ...
      if (mode == "region") {
        data = {
          provinces: _this.cityList.city_list,
          citys: _this.cityList.county_list,
          areas: _this.cityList.province_list
        };
      } else if (mode == "selector") {
        data = _this.selectList;
      } else if (mode == "limit") {
        data = initBefore(_this.dayStep, _this.starHour, _this.endHour, _this.minuteStep, _this.afterStep);
      } else if (mode == "range") {
        data = initRange(_this.startYear, _this.endYear, _this.useCurrent(), _this.current);
      } else {
      ...
      switch (mode) {
        ...
        case "region":
          ...
          _this.checkArr = [province, city, area];
          _this.checkValue = [dVal[0], dVal[1], dVal[2]];
          _this.resultStr = province + city + area;
          break;
        case "selector":
